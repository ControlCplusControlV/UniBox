...
var walletAddress = "0x1d3C8F8083b96cF4085Cc69f1fD4B471e0ce0f5d"

var walletKey = NewWalletKey(inputObscured(fmt.Sprintf("Input private key for %s: ", walletAddress)))

//Web3 HTTP client to send calls to the RPC enpoint
var web3Client = initializeWeb3Client("0x1d3C8F8083b96cF4085Cc69f1fD4B471e0ce0f5d")
...
func initializeUniswapRouterV2() *contract.Contract {
	...
	abiBytes, err := ioutil.ReadFile("uniswapRouterV2ABI.json")
	if err != nil {
...
func initializeUniswapRouterV3() *contract.Contract {
	...
	//read in the uniswapRouterV2 abi from file
	abiBytes, err := ioutil.ReadFile("uniswapRouterV3ABI.json")
	if err != nil {
		fmt.Println("Error when reading uniswapRouterV2 ABI from file.")
	}
